# l10x Pre-flight: lets-start-here
...
⚠  Context readiness score is below 50/100 — context is insufficient for reliable routing.
   Run `l10x context bootstrap` to set up service context before proceeding.
```
...
```bash
l10x classify --request "..." --format json
```
...
```bash
l10x suggest --request "..." --format json --top 3
```
